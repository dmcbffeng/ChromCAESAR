...
def upload_data(ch, pos, type, res):
    max_distance = 250000
    fname = '/nfs/turbo/umms-drjieliu/proj/4dn/data/microC/high_res_map_project_training_data/MicroC/{0}/{1}/{2}_{3}bp_{4}_{5}.npy'.format(
        type, ch, ch, res, pos, pos + max_distance)
    dest = 'gs://micro_c_training_data/MicroC/{0}/processed/{1}/{2}_{3}bp_{4}_{5}.npy'.format(
        type, ch, ch, res, pos, pos + max_distance)
    ...
    for res in resolutions:
        ...
        for pos in range(st, ed - max_distance + 1, 125000):
            ...
            if res != 200:
                ...
                m = f(new_co, new_co)
            fname = '/nfs/turbo/umms-drjieliu/proj/4dn/data/microC/high_res_map_project_training_data/MicroC/{0}/{1}/{2}_{3}bp_{4}_{5}.npy'.format(type, ch, ch, res, pos, pos + max_distance)
...
def process_data(type='mESC'):
    if type == 'mESC':
        ...
        path = '/nfs/turbo/umms-drjieliu/proj/4dn/data/microC/mESC/processed/hic_contacts/'
        rg = 'mm10'
    elif type == 'hESC':
        sizes = HUMAN_CHR_SIZES
        del sizes['chr1']
        del sizes['chr2']
        path = '/nfs/turbo/umms-drjieliu/proj/4dn/data/microC/human/processed/hic_contacts/'
        rg = 'hg38'
    else:
        raise ValueError

    for ch in sizes:
        file_path = path + ch + '_200bp.txt'
        contact_maps_from_a_processed_file(ch, file_path, rg, True,
                                           sizes, [200], type)
