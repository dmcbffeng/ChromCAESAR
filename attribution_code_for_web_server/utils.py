...
def load_all_data(ch, start_pos, signals):
    hic = load_hic_data(ch, start_pos)
    epi = load_epigenetic_data([ch], signals)
    epi = epi[ch][start_pos // 200 - 7: start_pos // 200 + 1257, :]
    return hic, epi


def load_hic_data(ch, start_pos):
    os.system(f'cd {path}')
    os.system(f'java -jar juicer_tools.jar dump observed NONE {path}/source_data/4DNFIQYQWPF5.hic\
                {ch}:{start_pos}:{start_pos+250000} {ch}:{start_pos}:{start_pos+250000} BP 1000 {path}/temp.txt')
    mat = np.zeros((250, 250))
    for line in open(f'{path}/temp.txt'):
...
def load_epigenetic_data(chromosomes, signals):
    ...
    for chrom in chromosomes:
        ...
        for i, k in enumerate(signals):
            s = np.load(f'{path}/source_data/pancreas_{chrom}_{k}_200bp.npy')
            s = zscore(s)
...
def model_fn(model_weights='v23_temp_model_25.h5',
             ...
             nBins=1250, nMarks=8, lr=0.0001, verbose=1):
    """
    Build the GCN model
...
def save_bigBed(attributions, signals, ch, start_pos):
    ...
    for i, signal in enumerate(signals):
        ...
        for j in range(len(s)):
            ...
            f.write(f'{ch} {start_pos + j * 200} {start_pos + j * 200 + 200} {s[j]}\n')

        f.close()

        # Convert into bigBed
        os.system(f'{path}/bedToBigBed {path}/{signal}_temp.bed {path}/mm10.chrom.sizes {path}/output_bigBed/{signal}_temp.bigBed')
        os.system(f'rm {path}/{signal}_temp.bed')
